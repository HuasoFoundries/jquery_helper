module.exports = function (grunt) {
	...
	var fs = require("fs"),
		...
		wrapper = grunt.file.read("src/helpers/wrapper_amd.js").split('@CODE');
	/**
	...
	grunt.config('concat', {
		...
		/*hammerjs: {
					options: {
						...
						process: function (src, filepath) {
							...
							if (filepath.indexOf('src/hammer.prefix.js') !== -1) {
								...
								var modified_src = "(function (global, factory) {";
								...
								var modified_src = src.split('var freeGlobal');
								return modified_src[0];

							} else if (filepath.indexOf('src/hammer.suffix.js') !== -1) {
								return "return Hammer;" + '\n' + "}));";
							} else {
								return src;
							}
				...
				tabs: {
					options: {
						process: function (src, filepath) {
							var modified_src = src.replace('(function ($) {', "define(['jquery'], function ($) { " + '\n' + "var jQuery=$; " + '\n');
							return modified_src.replace('}( jQuery ));', '});');
						}
					},
					src: ["node_modules/materialize-css/js/tabs.js"],
					dest: 'src/helpers/tabs.js'
				},
					options: {
						process: function (src, filepath) {

							var modified_src = (src.split('return function (global, window, document, undefined) {')[1]).split('}((window.jQuery || window.Zepto || window), window, document);'); //removes jquery shim

							if (filepath.indexOf('velocity.js') !== -1) {

								var modified_head = "(function (root, factory) {";
								modified_head = modified_head + '\n' + "typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('jquery')) :";
								modified_head = modified_head + '\n' + "typeof define === 'function' && define.amd ? define(['jquery'],factory) :";
								modified_head = modified_head + '\n' + "(root.Velocity = factory(root.jQuery));";
								modified_head = modified_head + '\n' + "}(this, function (global) {";
								modified_head = modified_head + '\n' + "'use strict';";
								modified_head = modified_head + '\n' + "var document = window.document;";

								modified_head = modified_head + '\n' + modified_src[0].replace('return Velocity;', '');
				...
				materialize: {
					options: {
						process: function (src, filepath) {
							...
							// Replaces head with my own code
							if (filepath.indexOf('animation.js') !== -1) {
	...
	grunt.registerTask('concates6', ['concat:hammeres6', 'concat:velocityes6', 'concat:materializees6']);

	grunt.registerTask('publish', ['custom:-effects,-deprecated,-manipulation/_evalUrl,-exports/amd,-exports/global', 'concates6', 'jspmbuild']);
