...
QUnit.test("jQuery.merge()", function (assert) {
	...
	assert.deepEqual(
		jQuery.merge([1], [2]), [1, 2],
		"Basic (single-element)"
	);
	assert.deepEqual(
		jQuery.merge([1, 2], [3, 4]), [1, 2, 3, 4],
	...
	// Fixed at [5998], #3641
	assert.deepEqual(
		jQuery.merge([-2, -1], [0, 1, 2]), [-2, -1, 0, 1, 2],
		"Second array including a zero (falsy)"
	);

	// After fixing #5527
	assert.deepEqual(
		jQuery.merge([], [null, undefined]), [null, undefined],
	...
	assert.deepEqual(
		jQuery.merge([], document.getElementById("lengthtest").getElementsByTagName("input")), [document.getElementById("length"), document.getElementById("idTest")],
		"Second NodeList"
	);
});
...
QUnit.test("jQuery.grep()", function (assert) {
	...
	assert.deepEqual(
		jQuery.grep([1, 2, 3, 4, 5, 6], searchCriterion), [2, 4, 6],
	...
	assert.deepEqual(
		jQuery.grep([1, 2, 3, 4, 5, 6], searchCriterion, true), [1, 3, 5],
		"Satisfying elements present and grep inverted"
	);
	assert.deepEqual(
		jQuery.grep([1, 3, 5, 7], searchCriterion, true), [1, 3, 5, 7],
		"Satisfying elements absent and grep inverted"
	);

	assert.deepEqual(
		jQuery.grep([1, 2, 3, 4, 5, 6], searchCriterion, false), [2, 4, 6],
...
QUnit.test("jQuery.extend(Object, Object)", function (assert) {
	...
	// #5991
	assert.ok(jQuery.isArray(jQuery.extend(true, {
		"arr": {}
